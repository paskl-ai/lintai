...
class ProjectAnalyzer:
    def __init__(self, units: List[PythonASTUnit], call_depth: int = 2):
        ...
        self._variable_map: Dict[str, str] = {} # Maps a variable name to its qualified type name
    ...
    def analyze(self) -> "ProjectAnalyzer":
        ...
        for unit in self.units:
            inventory = FileInventory(file_path=str(unit.path), frameworks=detect_frameworks(unit.tree), components=[])
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    self._import_trackers[path_str][alias.asname or alias.name.split('.')[0]] = alias.name
            elif isinstance(node, ast.ImportFrom) and node.module:
                for alias in node.names:
                    self._import_trackers[path_str][alias.asname or alias.name] = f"{node.module}.{alias.name}"
            elif isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                qualname = unit.qualname(node)
                self._qualname_to_node[qualname] = node
                self._call_graph.add_node(qualname)

    def _map_relationships(self, component: Component, node: ast.AST, unit: PythonASTUnit):
        ...
        if isinstance(node_to_inspect, ast.Call):
            ...
            for arg_node in args:
                if isinstance(arg_node, ast.Name):
                    ...
                    component.relationships.append(Relationship(target_name=arg_node.id, type="uses"))
    
        # Find "uses" relationships from the LCEL Pipe Operator
        elif isinstance(node_to_inspect, ast.BinOp) and isinstance(node_to_inspect.op, ast.BitOr):
            if isinstance(node_to_inspect.left, ast.Name):
                component.relationships.append(Relationship(target_name=node_to_inspect.left.id, type="uses"))
            if isinstance(node_to_inspect.right, ast.Name):
                component.relationships.append(Relationship(target_name=node_to_inspect.right.id, type="uses"))
        ...
        if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
            ...
            for sub_node in ast.walk(node):
                if isinstance(sub_node, ast.Call):
                    callee_qualname = self._resolve_call_to_qualname(sub_node.func, unit)
                    if callee_qualname:
                        ...
                        if self._call_graph.has_node(callee_qualname):
                            ...
                            component.relationships.append(Relationship(target_name=callee_qualname, type="calls"))
        
            # 2. Find "uses" relationships for the function's parameters (the new logic)
            for arg in node.args.args:
                if arg.arg == 'self':
                    continue
                component.relationships.append(Relationship(target_name=arg.arg, type="uses"))

    def _resolve_call_to_qualname(self, func_node: ast.AST, unit: PythonASTUnit) -> Optional[str]:
    ...
    def _node_to_component(self, node: ast.AST, unit: PythonASTUnit) -> Optional[Component]:
        """
        ...
        if isinstance(node, ast.Call):
            if not isinstance(node.func, (ast.Name, ast.Attribute)): return None
            ...
            if len(node.targets) == 1 and isinstance(node.targets[0], ast.Name):
                ...
                if isinstance(node.value, ast.Call):
                    value_name = get_full_attr_name(node.value.func)
                elif isinstance(node.value, (ast.BinOp, ast.Dict, ast.List, ast.Constant)):
                     # Heuristic: if it's a chain or complex object, classify the variable itself
                     value_name = node.targets[0].id

                value_type = classify_component_type(value_name)
                if value_type not in ["Unknown", "Ignore"]:
